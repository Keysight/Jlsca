...
function testKeyExpansion128()
    ...
    @test(w[end-8+1:end] == hex2bytes("e13f0cc8b6630ca6"))
end
...
function testKeyExpansion192()
    ...
    @test(w[end-8+1:end] == hex2bytes("8ecc720401002202"))
end
...
function testKeyExpansion256()
    ...
    @test(w[end-8+1:end] == hex2bytes("046df344706c631e"))
end
...
function testMixColumns()
    ...
    state = state
    state = permutedims(reshape(state, (4,4)))
    state1 = MixColumns(state)
    state2 = permutedims(reshape(hex2bytes(replace("04 e0 48 28 66 cb f8 06 81 19 d3 26 e5 9a 7a 4c", " " => "")), (4,4)))
    state2 = state2
    @test(state1 == state2)
...
function testShiftRows()
    state = permutedims(reshape(hex2bytes(replace("d4 e0 b8 1e 27 bf b4 41 11 98 5d 52 ae f1 e5 30", " " => "")), (4,4)))
    state1 = ShiftRows(state)
    state2 = permutedims(reshape(hex2bytes(replace("d4 e0 b8 1e bf b4 41 27 5d 52 11 98 30 ae f1 e5", " " => "")), (4,4)))
    @test(state1 == state2)
...
function testAddRoundKey()
    state = permutedims(reshape(hex2bytes(replace("58 1b db 1b 4d 4b e7 6b ca 5a ca b0 f1 ac a8 e5", " " => "")), (4,4)))
    key = permutedims(reshape(hex2bytes(replace(" f2 7a 59 73 c2 96 35 59 95 b9 80 f6 f2 43 7a 7f", " " => "")), (4,4)))
    state1 = AddRoundKey(state, key)
    state2 = permutedims(reshape(hex2bytes(replace("aa 61 82 68 8f dd d2 32 5f e3 4a 46 03 ef d2 9a", " " => "")), (4,4)))
...
function testSubBytes()
    state = permutedims(reshape(hex2bytes(replace("aa 61 82 68 8f dd d2 32 5f e3 4a 46 03 ef d2 9a", " " => "")), (4,4)))
    state1 = SubBytes(state)
    state2 = permutedims(reshape(hex2bytes(replace("ac ef 13 45 73 c1 b5 23 cf 11 d6 5a 7b df b5 b8", " " => "")), (4,4)))
    @test(state1 == state2)
